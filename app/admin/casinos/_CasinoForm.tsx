...
const LICENSE_TRUST_SCORES: Record<string, number> = {
  "UKGC": 90,
  "MGA": 85, "Spelinspektionen": 85,
  "Danish Gambling Authority (Spillemyndigheden)": 85, "Norwegian Gaming Authority (Lotteritilsynet)": 85,
  "Belgian Gaming Commission": 85, "Estonian Tax and Customs Board (EMTA)": 85,
  "Gibraltar": 80, "Isle of Man": 80, "Veikkaus": 80,
  "Finnish National Police Board (Poliisihallitus)": 80,
  "Latvian Lotteries and Gambling Supervisory Inspection": 80, "Lithuanian Gaming Control Authority": 80,
  "Romanian National Gambling Office (ONJN)": 75,
...
function TrustScoreField({ value, onChange, licenseAuthority, establishedYear, liveChatSupport, mobileOptimized, vipProgram }: {
  ...
  licenseAuthority?: string | null; establishedYear?: number | null
  liveChatSupport?: boolean | null; mobileOptimized?: boolean | null; vipProgram?: boolean | null
}) {
    ...
    <div>
      ...
      {suggestion !== null && suggestion !== value && (
        <div className="mt-2 flex items-center gap-2 text-xs bg-[#2D1783]/5 border border-[#2D1783]/15 rounded-lg px-3 py-2">
          ...
          <button type="button" onClick={() => onChange(suggestion)} className="text-[10px] font-bold text-[#2D1783] hover:underline flex-shrink-0">Apply</button>
        </div>
      )}
...
function ContentTab({ lang, form, onChange }: {
  lang: "fi" | "en" | "uk"; form: Casino; onChange: (patch: Partial<Casino>) => void
}) {
...
function AiPopulateTab({ casinoName, casinoSlug, casinoId, currentForm, onApply }: {
  ...
  const generateContent = async () => {
    setContentLoading(true); setContentError(null); setContentMessage(null)
    setContentApiResponse(null); setContentApplyMsg(null); setContentApplyError(null); setContentProgress(null)
    try {
      ...
      for (const lang of langs) {
        ...
        try { json = await res.json() } catch { throw new Error(`${langLabel} timed out — try one language at a time`) }
        if (!res.ok) throw new Error(json.error || `${langLabel} generation failed`)
        if (json.message) skipMessages.push(json.message)
        if (json.data && Object.keys(json.data).length > 0) {
          Object.assign(allData, json.data); anyGenerated = true
        }
      }
      if (!anyGenerated) {
      ...
      setContentLoading(false); setContentProgress(null)
    }
  }
  ...
  const saveContentToDb = async (patch: Record<string, unknown>): Promise<boolean> => {
    ...
    setContentSaving(true); setContentApplyError(null)
    try {
      ...
      setContentApplyError(e instanceof Error ? e.message : "Save failed"); return false
  ...
  const doContentApply = async (patch: Record<string, unknown>, count: number) => {
    ...
    if (ok) { setContentApplyMsg(`Applied & saved ${count} field${count !== 1 ? "s" : ""} to Supabase`); setTimeout(() => setContentApplyMsg(null), 5000) }
  }
  ...
  const saveToDb = async (patch: Record<string, unknown>): Promise<boolean> => {
    if (!casinoId) { setApplyError("Casino ID missing — save the casino record first, then retry."); return false }
    ...
    try {
      ...
      setApplyError(e instanceof Error ? e.message : "Save failed"); return false
  ...
  const doApply = async (patch: Record<string, unknown>, count: number) => {
    ...
    if (ok) { setApplyMsg(`Applied & saved ${count} field${count !== 1 ? "s" : ""} to Supabase`); setTimeout(() => setApplyMsg(null), 5000) }
  }
  ...
  const diffItems = apiResponse ? Object.entries(apiResponse.data)
    ...
    .map(([key, newVal]) => {
      ...
      const status: "new" | "update" | "same" =
        isEmptyValue(currentVal) ? "new" : JSON.stringify(currentVal) !== JSON.stringify(newVal) ? "update" : "same"
      ...
      {apiResponse && (
        <>
            ...
            {diffItems.length === 0 ? (
              ...
              <div className="space-y-1">
                <div className="flex items-center gap-3 pb-2 mb-1 border-b border-[#E5E8F0] sticky top-0 bg-white z-10">
                  <button type="button" onClick={() => setSelectedKeys(new Set(diffItems.filter(i => i.status !== "same").map(i => i.key)))} className="text-xs font-bold text-[#2D1783] hover:underline">Select changed</button>
                  <button type="button" onClick={() => setSelectedKeys(new Set(diffItems.map(i => i.key)))} className="text-xs font-bold text-[#2D1783] hover:underline">Select all</button>
                  <button type="button" onClick={() => setSelectedKeys(new Set())} className="text-xs font-bold text-[#787585] hover:underline">Clear</button>
                    ...
                    return (
                      <label key={key} className={`flex items-center gap-2.5 px-3 py-2 rounded-lg border cursor-pointer hover:opacity-90 transition-opacity ${s.bg}`}>
                        <input type="checkbox" checked={selectedKeys.has(key)}
                          onChange={e => { const next = new Set(selectedKeys); e.target.checked ? next.add(key) : next.delete(key); setSelectedKeys(next) }}
          ...
          <div className="space-y-3">
            ...
            {applyMsg && <div className="flex items-center gap-2 text-sm text-[#27AE60] font-medium"><span className="material-symbols-outlined text-[16px]">check_circle</span>{applyMsg}</div>}
            {applyError && (
              <div className="flex items-center gap-2 text-sm text-red-600 font-medium">
                <span className="material-symbols-outlined text-[16px]">error</span>
                <span className="flex-1">{applyError}</span>
                <button type="button" onClick={() => setApplyError(null)} className="text-red-400 hover:text-red-600 flex-shrink-0"><span className="material-symbols-outlined text-[14px]">close</span></button>
              </div>
            )}
          </div>
        </>
      ...
      <SectionCard title="✍️ Generate Written Content" icon="article">
        <div className="space-y-5">
                ...
                return (
                  <label key={lang} className="flex items-center gap-2 cursor-pointer select-none">
                    <div className="relative">
                      <input type="checkbox" checked={contentLanguages.has(lang)}
                        onChange={e => { const next = new Set(contentLanguages); e.target.checked ? next.add(lang) : next.delete(lang); setContentLanguages(next) }}
                        className="sr-only" />
          ...
          <label className="flex items-center gap-2.5 cursor-pointer select-none group">
            <div className="relative">
              <input type="checkbox" checked={regenerateExisting} onChange={e => setRegenerateExisting(e.target.checked)} className="sr-only" />
          ...
          <button type="button" onClick={generateContent} disabled={contentLoading || contentLanguages.size === 0}
      ...
      {contentApiResponse && contentDiffItems.length > 0 && (
        <>
          <SectionCard title={`Written Content — ${contentNewCount} new, ${contentUpdateCount} updates`} icon="difference">
            <div className="space-y-1">
              <div className="flex items-center gap-3 pb-2 mb-1 border-b border-[#E5E8F0] sticky top-0 bg-white z-10">
                <button type="button" onClick={() => setContentSelectedKeys(new Set(contentDiffItems.filter(i => i.status !== "same").map(i => i.key)))} className="text-xs font-bold text-[#2D1783] hover:underline">Select changed</button>
                <button type="button" onClick={() => setContentSelectedKeys(new Set(contentDiffItems.map(i => i.key)))} className="text-xs font-bold text-[#2D1783] hover:underline">Select all</button>
                <button type="button" onClick={() => setContentSelectedKeys(new Set())} className="text-xs font-bold text-[#787585] hover:underline">Clear</button>
                  ...
                  return (
                    <label key={key} className={`flex items-center gap-2.5 px-3 py-2 rounded-lg border cursor-pointer hover:opacity-90 transition-opacity ${s.bg}`}>
                      <input type="checkbox" checked={contentSelectedKeys.has(key)}
                        onChange={e => { const next = new Set(contentSelectedKeys); e.target.checked ? next.add(key) : next.delete(key); setContentSelectedKeys(next) }}
                        className="w-3.5 h-3.5 accent-[#2D1783] flex-shrink-0" />
                      <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded flex-shrink-0 ${s.badge}`}>{s.label}</span>
                      <span className="text-[11px] font-mono text-[#474554] flex-shrink-0 w-36 truncate">{key}</span>
                      <span className="text-[11px] text-[#787585] flex-1 truncate">{formatAiValue(newVal)}</span>
          ...
          <div className="space-y-3">
            <div className="flex gap-3">
              <button type="button" onClick={applyContentSelected} disabled={contentSelectedKeys.size === 0 || contentSaving}
                className="flex-1 flex items-center justify-center gap-2 bg-[#2D1783] text-white font-bold py-2.5 rounded-xl hover:bg-[#3e2db2] disabled:opacity-40 disabled:cursor-not-allowed transition-colors text-sm">
                {contentSaving ? <span className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" /> : <span className="material-symbols-outlined text-[16px]">checklist</span>}
                Apply Selected ({contentSelectedKeys.size})
              </button>
              <button type="button" onClick={applyContentAll} disabled={contentSaving}
                className="flex-1 flex items-center justify-center gap-2 bg-[#27AE60] text-white font-bold py-2.5 rounded-xl hover:bg-[#219a52] disabled:opacity-50 disabled:cursor-not-allowed transition-colors text-sm">
                {contentSaving ? <span className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" /> : <span className="material-symbols-outlined text-[16px]">done_all</span>}
                {contentSaving ? "Saving..." : "Apply All & Save"}
              </button>
            </div>
            {contentApplyMsg && <div className="flex items-center gap-2 text-sm text-[#27AE60] font-medium"><span className="material-symbols-outlined text-[16px]">check_circle</span>{contentApplyMsg}</div>}
            {contentApplyError && (
              <div className="flex items-center gap-2 text-sm text-red-600 font-medium">
                <span className="material-symbols-outlined text-[16px]">error</span>
                <span className="flex-1">{contentApplyError}</span>
                <button type="button" onClick={() => setContentApplyError(null)} className="text-red-400 hover:text-red-600 flex-shrink-0"><span className="material-symbols-outlined text-[14px]">close</span></button>
              </div>
            )}
          </div>
        </>
...
// ─── EMPTY_CASINO factory ────────────────────────────────────────────────────

const EMPTY_CASINO = (slug: string): Casino => ({
  id: "", slug, name: "", rank: 99, rating: 7.0, trust_score: 70,
...
export function CasinoForm({ slug, createMode = false }: { slug?: string; createMode?: boolean }) {
  ...
  const handleNameChange = (name: string) => {
    if (createMode && !slugManual) {
      ...
      patch({ name, slug: auto, mene_slug: auto })
    } else {
      patch({ name })
    }
  }
  ...
  const save = async () => {
    ...
    try {
      ...
      if (createMode && !form.id) {
        endpoint = "/api/admin/casinos"
        method = "POST"
      } else if (form.id) {
        endpoint = `/api/admin/casinos/${form.id}`
        method = "PATCH"
      } else {
        endpoint = `/api/admin/casinos/${slug}`
        method = "POST"
      }
      ...
      if (createMode && updated?.slug) {
        router.push(`/admin/casinos/${updated.slug}/edit`)
        return
      }
    ...
    <div>
            ...
            <div>
              <div className="flex items-center gap-2">
                <h1 className="font-display font-bold text-lg text-[#1b1b1c]">
                  {createMode ? (form.name || "New Casino") : form.name}
                </h1>
                {!createMode && (
                ...
                {createMode && (
                  <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-[#2D1783]/10 text-[#2D1783]">New</span>
                )}
              </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <button type="button" onClick={save} disabled={saved === "saving"}
              ...
              {saved === "ok" ? "Saved!" : saved === "error" ? "Error!" : saved === "saving" ? "Saving..."
                : createMode ? "Create Casino" : "Save Changes"}
            </button>
          </div>
        </div>
      </div>
      ...
      <div className="flex gap-6">
        ...
        <div className="flex-1 min-w-0 space-y-4">
            ...
            <>
              <SectionCard title="Basic Info" icon="badge">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <Input label="Casino Name" value={form.name} onChange={e => handleNameChange(e.target.value)} placeholder={createMode ? "e.g. Casumo Casino" : ""} />
                  <div>
                    <div className="flex items-center justify-between mb-1">
                      <Label>Slug</Label>
                      {createMode && (
                        <span className="text-[10px] text-[#787585]">{slugManual ? "manual" : "auto"}</span>
                      )}
                    </div>
                    <input
                      value={form.slug}
                      onChange={e => { setSlugManual(true); patch({ slug: e.target.value, mene_slug: e.target.value }) }}
                      className="w-full bg-[#F8F9FD] border border-[#E5E8F0] focus:border-[#2D1783] focus:outline-none rounded-xl px-3 py-2 text-sm text-[#1b1b1c] placeholder:text-[#b0adb8] transition-colors"
                    />
                      ...
                      <div className="md:col-span-2">
                        ...
                        <select value={dropdownVal}
                          onChange={e => {
                            ...
                            if (score > 0 && (!form.trust_score || form.trust_score === 70)) patch({ trust_score: score })
                          }}
                          className="w-full bg-[#F8F9FD] border border-[#E5E8F0] focus:border-[#2D1783] focus:outline-none rounded-xl px-3 py-2 text-sm">
                        ...
                        {(dropdownVal === "Other" || isCustom) && (
                          <input type="text" placeholder="Enter license authority name..."
                        ...
                        {suggestedScore > 0 && (
                          <div className="mt-1.5 flex items-center gap-2">
                            ...
                            <button type="button" onClick={() => patch({ trust_score: suggestedScore })}
                              className="text-[11px] font-bold text-[#2D1783] hover:underline">{suggestedScore} (apply)</button>
                          </div>
                        )}
                      </div>
                      <div>
                ...
                <div className="space-y-4">
                  ...
                  {form.kyc_required && (
                    <CheckboxGrid label="KYC Documents" items={["ID", "Proof of Address", "Payment Proof"]}
                      selected={form.kyc_documents ?? []} onChange={v => patch({ kyc_documents: v })} />
                  )}
            ...
            <>
              <SectionCard title="Logo" icon="image">
                <div className="flex items-start gap-5">
                  <MediaUpload
                    value={form.logo_url} slug={form.slug}
              ...
              <SectionCard title="Banner / Hero Image" icon="panorama">
                <MediaUpload
                  value={form.banner_url} slug={form.slug}
          ...
          {activeTab === "ai" && (
            <AiPopulateTab
              casinoName={form.name}
              casinoSlug={form.slug}
              casinoId={form.id}
              currentForm={form}
              onApply={patch}
            />
